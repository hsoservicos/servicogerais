            ...
            <div class="bg-white rounded-2xl shadow-modal border border-border overflow-hidden mb-6">
                ...
                <div class="px-6 sm:px-8 py-6 border-b border-border">
                    <div class="flex items-center justify-between mb-4">
                        <p class="text-sm font-semibold text-ink">Itens da Proposta</p>
                        <button onclick="downloadPublicPdf()"
                            class="inline-flex items-center gap-1.5 text-xs font-medium text-danger hover:text-danger/80 transition-colors">
...
function downloadPublicPdf() {
    const url = `/api/v1/public/proposals/${encodeURIComponent(TOKEN)}/pdf`;
    const link = document.createElement('a');
    link.href = url;
    link.target = '_blank';
    link.download = `proposta-${proposalData?.number || TOKEN.slice(0, 8)}.pdf`;
