...
/**
 * ═══════════════════════════════════════════════════════════════
 * templates/categories.php — Gestão de Categorias (CRUD)
 * ═══════════════════════════════════════════════════════════════
 */
...
<!-- ═══════════════════════════════════════════════════════════════
     templates/categories.php — Gestão de Categorias
     ═══════════════════════════════════════════════════════════════ -->
<?php $currentPage = 'categories'; require __DIR__ . '/partials/sidebar.php'; ?>

<!-- Main Content -->
<div class="md:ml-64 min-h-screen flex flex-col">
    <?php
    $pageTitle = 'Categorias';
    $pageSubtitle = 'Organize seus serviços em categorias';
    $topbarExtra = '<button onclick="openCreateModal()" class="bg-primary text-white font-medium px-4 py-2 rounded-lg hover:bg-primary-600 active:bg-primary-700 transition-all duration-200 flex items-center gap-2 shadow-card text-sm"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg> Nova Categoria</button>';
    ...
    <main class="flex-1 p-6">
        ...
        <div class="flex flex-col sm:flex-row gap-4 mb-6">
            <div class="relative flex-1">
                ...
                <input type="text" id="search-input" placeholder="Buscar categoria..."
    ...
    <div class="relative min-h-screen flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-modal w-full max-w-md animate-fade-in">
            <div class="flex items-center justify-between px-6 py-4 border-b border-border">
                <h3 id="modal-title" class="text-h3 text-ink">Nova Categoria</h3>
            ...
            <form id="category-form" class="p-6 space-y-4" novalidate>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="field-icon"
                            class="w-full px-4 py-2.5 rounded-lg border border-border bg-surface text-ink placeholder:text-ink-muted focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"
                            placeholder="scissors, brush...">
                    </div>
                    <div>
...
<div id="delete-modal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="relative min-h-screen flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-modal w-full max-w-sm animate-fade-in p-6 text-center">
            ...
            <h3 class="text-h3 text-ink mb-2">Desativar Categoria?</h3>
            <p class="text-ink-secondary text-sm mb-6" id="delete-message">Os serviços desta categoria não serão removidos.</p>
...
async function loadCategories() {
    ...
    try {
        ...
        renderTable(data.categories, data.pagination);
    } catch (err) {
        table.innerHTML = `<div class="text-center py-12"><p class="text-danger">❌ ${err.message}</p></div>`;
    }
}

function renderTable(categories, pagination) {
    const table = document.getElementById('categories-table');
    document.getElementById('pagination').classList.toggle('hidden', !categories || categories.length === 0);
    ...
    if (!categories || categories.length === 0) {
        table.innerHTML = `
            <div class="text-center py-16 px-6">
                ...
                <h3 class="text-h3 text-ink mb-2">Nenhuma categoria cadastrada</h3>
                <p class="text-ink-secondary text-sm mb-6">Crie categorias para organizar seus serviços.</p>
                <button onclick="openCreateModal()" class="bg-primary text-white font-medium px-6 py-2.5 rounded-lg hover:bg-primary-600 transition-all">+ Nova Categoria</button>
            </div>`;
        return;
    }
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-surface/80 border-b border-border">
                    ...
                    <th class="text-left px-6 py-3 text-xs font-semibold text-ink-muted uppercase tracking-wider hidden md:table-cell">Descrição</th>
                    <th class="text-center px-6 py-3 text-xs font-semibold text-ink-muted uppercase tracking-wider">Cor</th>
                    ...
                    <tr class="hover:bg-surface/50 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-ink-muted">${c.icon || '—'}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <button onclick="openDeleteModal(${c.id})" title="Desativar"
    ...
    document.getElementById('pagination-info').textContent =
        `Mostrando ${categories.length} de ${pagination.total} categorias (página ${pagination.page} de ${pagination.totalPages})`;
...
document.getElementById('field-color').addEventListener('input', (e) => {
    document.getElementById('field-color-hex').value = e.target.value;
});
document.getElementById('field-color-hex').addEventListener('input', (e) => {
...
document.getElementById('category-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const payload = {
            ...
            icon: document.getElementById('field-icon').value.trim() || null,
            color: document.getElementById('field-color-hex').value.trim() || null,
        };

        const url = isEdit ? `${API_BASE}/categories/${categoryId}` : `${API_BASE}/categories`;
        ...
        btn.disabled = false;
        btn.innerHTML = isEdit ? 'Atualizar' : 'Salvar Categoria';
    }
});
...
document.getElementById('confirm-delete-btn').addEventListener('click', async () => {
    ...
    try {
        const token = '<?= getToken() ?>';
        const response = await fetch(`${API_BASE}/categories/${deleteCategoryId}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await response.json();
